    ...
    <section className="w-full flex flex-col items-center justify-center py-8 sm:py-12" id="about">
      <div className="max-w-6xl w-full">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">         
          <SpotlightCard className="md:col-span-2 p-6 sm:p-4 backdrop-blur-xl group animate-fade-in-up">
              ...
              <div className="grid grid-cols-3 sm:grid-cols-6 md:gap-4 items-center justify-items-center w-full">
                {techStack.map((tech, index) => (
                  ...
                  >
                    ...
                    <span className="absolute bottom-0 text-xs font-semibold text-white opacity-0 group-hover/icon:opacity-100 transition-all duration-300 transform translate-y-2 group-hover/icon:translate-y-0">
                      {tech.name}
                    </span>
                  </div>
                ))}
              ...
              <div className="flex flex-wrap gap-3 justify-center md:justify-end">
                {socialLinks.map((social) => (
                  <a
                    ...
                    className={`flex items-center gap-3 px-4 py-2 sm:px-6 sm:py-3 bg-white/5 rounded-full border border-white/5 hover:border-white/20 hover:bg-white/10 transition-all text-neutral-400 ${social.color} z-30 relative`}
                  >
                    {social.icon}
